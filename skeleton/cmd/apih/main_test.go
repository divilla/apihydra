...
func TestFatalDiagnosticInvalidKindIsExact(t *testing.T) {
	err := errs.WithExitCode(errs.ExitConfiguration, definition.ErrInvalidKind)
	want := "error: kind: must be one of: <root|defaults|steps>\n\n" +
		"please check user manual: " + userManualReference + "#invalid-yaml-definition\n"
	if got := fatalDiagnostic(err); got != want {
		t.Fatalf("fatalDiagnostic() = %q, want %q", got, want)
	}
	if !errors.Is(err, definition.ErrInvalidKind) || errs.Code(err, 0) != 102 {
		t.Fatal("kind error lost its identity or configuration code")
	}
}
...
func TestTroubleshootingAnchorClassifiesFatalErrors(t *testing.T) {
	tests := map[string]struct {
		...
		"root missing":       {definition.ErrRootDefinitionMissing, "root-defaults-file-missing"},
		...
		"invalid kind":       {definition.ErrInvalidKind, "invalid-yaml-definition"},
