...
expected_fix_prompt="\$change-code $specification Treat the review findings supplied via stdin as feedback against that specification. Validate each finding against the specification and repository evidence, then fix every valid finding completely, including meaningful tests and required verification. If no repository change can be made, explain exactly why in the final response. Preserve unrelated changes. Do not commit or push; the caller handles commits."
(
	cd "$repo"
	PATH="$fake_bin:$PATH" \
