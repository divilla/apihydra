...
- Production output: `internal/execution/varproc.go` replaces all canonical
  zero-value TODO bodies with the four binding variable phases while retaining
  the exact constructor and method signatures.
- Test output: `internal/execution/varproc_test.go` covers loading, both
...
- Each acceptance criterion is traced to at least one meaningful unit test, and
  VariableProcessor unit-test statement coverage remains greater than 95%.
